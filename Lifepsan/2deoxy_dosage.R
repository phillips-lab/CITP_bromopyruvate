...
comps <- read_excel("2deoxy_dosage.xlsx")
...
levels(comps$Compound)[levels(comps$Compound) == "2_deoxyglucose"] <- "deoxyglucose"
...
strain <- c("ED3092","N2_PD1073")
header <- c("ed3092","n2_pd1073")
...
for (i in ivector) {
  assign(paste(header[i],"cox",sep="_"),coxme(Surv(DeathAge,Dead) ~ Comp_Conc + (1|StartDate/Tech/plate_id), data=get(paste(header[i],sep="_"))))
...
for (i in ivector) {
  assign(paste(header[i],"lm",sep="_"),lmer(DeathAge ~ Comp_Conc + (1|StartDate/Tech/plate_id), data=get(paste(header[i],sep="_"))))
